...
import { Trash2, Eye, Reply, X } from "lucide-react";
import { api } from "@/hooks/api";
import { toast } from "sonner";

function openMailto(contact: any) {
  const subject = encodeURIComponent(`Re: ${contact.subject} — TradeFlow`);
...
export default function AdminContacts() {
  ...
  const handleStatusChange = async (id: number, newStatus: string) => {
    try {
      await api.patch(`/api/contacts/messages/${id}/`, { status: newStatus });
      fetchContacts();
    ...
    <div className="space-y-6">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              ...
              <tbody>
                {contacts.length === 0 && (
                  <tr>
                    <td colSpan={6} className="text-center py-8 text-slate-400">No messages found</td>
                  </tr>
                )}
                {contacts.map((c) => (
                  <tr key={c.id} className="border-b border-slate-700/30 hover:bg-slate-700/20 transition-colors">
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center gap-1">
                        ...
                        <button onClick={() => openMailto(c)} title="Reply via email"
      ...
      {selected && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 px-4">
          <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6 w-full max-w-lg space-y-3">
            <div className="flex items-center justify-between mb-1">
              <h3 className="text-lg font-bold text-white">Message from {selected.name}</h3>
            ...
            <div className="space-y-2 text-sm">
              ...
              <div>
                ...
                <p className="text-white mt-1.5 bg-slate-900/50 rounded-lg p-3 leading-relaxed">{selected.message}</p>
              </div>
            </div>
            <div className="flex gap-3 pt-2">
              <button
                onClick={() => { setSelected(null); openMailto(selected); }}
                className="flex-1 flex items-center justify-center gap-2 bg-slate-700 hover:bg-slate-600 text-white font-medium py-2.5 rounded-xl text-sm transition-colors"
              >
                <Reply className="w-4 h-4" /> Reply
              </button>
              <button
                onClick={() => setSelected(null)}
                className="flex-1 bg-[#D4AF37] text-[#101828] font-bold py-2.5 rounded-xl hover:bg-[#D4AF37]/90 transition-colors text-sm"
              >
                Close
              </button>
            </div>
