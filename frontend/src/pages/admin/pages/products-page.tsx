...
import { Trash2, Plus, Edit2, X, Eye, EyeOff, Save, Loader2 } from "lucide-react";
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 px-4 py-8 overflow-y-auto">
      <div className="bg-slate-800 border border-slate-700 rounded-2xl w-full max-w-lg overflow-hidden">
        <div className="relative w-full h-52 bg-slate-900">
          ...
          <button onClick={onClose} className="absolute top-3 right-3 bg-black/50 hover:bg-black/70 text-white rounded-full p-1.5"><X className="w-4 h-4" /></button>
          <span className={`absolute top-3 left-3 text-xs px-2.5 py-1 rounded-full font-medium ${product.category === "import" ? "bg-blue-500/80 text-white" : "bg-green-500/80 text-white"}`}>{product.category}</span>
        ...
        <div className="p-5 space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3 text-xs">
            <div className="bg-slate-900/50 rounded-lg p-3"><p className="text-slate-400 mb-1">Price</p><p className="text-white font-medium">{product.price || "—"}</p></div>
            <div className="bg-slate-900/50 rounded-lg p-3"><p className="text-slate-400 mb-1">Rating</p><p className="text-[#D4AF37] font-medium">{"★".repeat(Math.round(product.rating))} {product.rating}</p></div>
            <div className="bg-slate-900/50 rounded-lg p-3"><p className="text-slate-400 mb-1">Submitted by</p><p className="text-white font-medium">{product.submitted_by_username || "—"}</p></div>
            <div className="bg-slate-900/50 rounded-lg p-3"><p className="text-slate-400 mb-1">Published by</p><p className="text-white font-medium">{product.published_by_username || "—"}</p></div>
          </div>
          <button onClick={onClose} className="w-full bg-slate-700 hover:bg-slate-600 text-white font-medium py-2.5 rounded-xl text-sm">Close</button>
        </div>
      </div>
    </div>
  );
...
function ProductsCRUD() {
  ...
  const emptyForm: { name: string; description: string; image: string; category: "import" | "export"; type: string; price: string; rating: number; reviews: number; order: number; is_active: boolean } = {
    name: "", description: "", image: "", category: defaultCategory as "import" | "export", type: "", price: "", rating: 4.5, reviews: 0, order: 0, is_active: true,
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      else { await api.post("/api/content/products/", payload); toast.success(staffOnly ? "Submitted for review" : "Created"); }
              ...
              {items.map(item => (
                <tr key={item.id} className="border-b border-slate-700/20 hover:bg-slate-700/20">
                  ...
                  <td className="px-4 py-2.5">
                    {item.is_published ? <span className="text-xs px-2 py-0.5 rounded-full bg-green-500/20 text-green-400">Published</span> : <span className="text-xs px-2 py-0.5 rounded-full bg-yellow-500/20 text-yellow-400">Pending</span>}
                  </td>
                  <td className="px-4 py-2.5"><div className="flex gap-2 items-center">
                    ...
                    <button onClick={() => setViewing(item)} className="p-1 text-slate-400 hover:text-blue-400"><Eye className="w-4 h-4" /></button>
          ...
          <div className="bg-slate-800 border border-slate-700 rounded-2xl p-6 w-full max-w-2xl">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                ...
                <F label="Image URL" name="image" form={form} setForm={setForm} full />
                <div>
                ...
                <F label="Price" name="price" form={form} setForm={setForm} />
              ...
              {staffOnly && <p className="text-xs text-yellow-400/80 bg-yellow-500/10 rounded-lg px-3 py-2">Your product will be reviewed by your manager before going live.</p>}
...
function PageSettings() {
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-base font-semibold text-white">Products Page Settings</h3>
      ...
      <div className="bg-slate-900/40 border border-slate-700/50 rounded-xl p-4">
        <p className="text-xs text-slate-500 uppercase tracking-wider mb-3">Homepage Products Section</p>
...
export default function ProductsAdminPage() {
  ...
  return (
    <div className="space-y-6 max-w-5xl">
      <h2 className="text-2xl font-bold text-white">Products Page</h2>
